...
<head>
<meta charset="EUC-KR">
<title>userMainOrnamental</title>
<style>
        .navi {
    ...
    <form action="" name="frm">
    	<div>
        	<select name="sortlist" onchange="location=document.frm.sortlist.value">
            	...
            	<option value="userMainTeen.jsp">청소년 인기순</option>
            	<option value="userMainYouth.jsp">청년 인기순</option>
            	<option value="userMainMid.jsp">중년 인기순</option>
            	<option value="userMainOld.jsp">장년 인기순</option>
            	<option value="userMainDay.jsp">하루 인기순</option>
            	<option value="userMainMonth.jsp">한달 인기순</option>
            	<option value="userMainYear.jsp">일년 인기순</option>
        	</select><br>
    	</div>
    </form>
    ...
    try {
    	String sql = "SELECT\r\n"
    			...
    			+ "FROM(\r\n"
    			     + "SELECT SeedName AS TEEN_SEEDNAME, COUNT(*) AS VARIETYCOUNT, SUM(Quantity) AS Total_QUANTITY\r\n"
    			     + "FROM (\r\n"
    			          + "SELECT OD_UserID, OD_VarietyID, Quantity, SeedName, SeedPurPose\r\n"
    			          + "FROM (\r\n"
    			                 + "SELECT OD.OD_USERID, OD.OD_VarietyID, OD.Quantity, S.SeedName, S.SeedPurPose\r\n"
    			                 + "FROM \"ORDER\" OD INNER  JOIN SEED S ON OD.OD_VarietyID = S.VarietyID)\r\n"
    			          + "WHERE SeedPurpose = '관상용')\r\n"
    			     + "GROUP BY OD_VarietyID, SeedName\r\n"
    			     + "ORDER BY COUNT(OD_VarietyID) DESC)";		     
    	rs = stmt.executeQuery(sql);
        while (rs.next()) {
